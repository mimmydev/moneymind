...
const statCards = computed(() => [
  ...
  },
  {
    ...
    trendText: getTrend(
      centsToRM(analytics.value?.spendingTrends?.peakSpendingDay?.amount),
      'peakSpendingDay'
    ).text,
    trendIcon: getTrend(
      centsToRM(analytics.value?.spendingTrends?.peakSpendingDay?.amount),
  ...
  },
  {
    ...
    trendText: getTrend(
      centsToRM(analytics.value?.spendingTrends?.dailySpending?.amount),
      'dailyAverage'
    ).text,
    trendIcon: getTrend(
      centsToRM(analytics.value?.spendingTrends?.dailySpending?.amount),
...
const fetchExpenses = async () => {
  try {
    ...
    const merchantTotals = data.reduce(
      (acc, expense) => {
...
onMounted(async () => {
  await Promise.all([fetchAnalytics(), fetchExpenses()]);
});
</script>
