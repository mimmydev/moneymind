...
export class ExpenseRepository {
  ...
  async createExpense(userId: string, expenseData: CreateExpenseRequest): Promise<Expense> {
    ...
    const category =
      ...
      this.autoCategorizeMalaysian(expenseData.description, expenseData.amount);
    ...
    const expense: ExpenseItem = {
      ...
      confidence: expenseData.category
        ? 100
        : this.getCategorizationConfidence(expenseData.description),
      createdAt: now,
      updatedAt: now,
    };
   ...
   */
  async searchExpensesByDescription(
    userId: string,
    searchTerm: string,
    limit: number = 20
  ): Promise<Expense[]> {
  ...
  private autoCategorizeMalaysian(description: string, amount: number): MalaysianCategory {
    ...
    for (const [pattern, keywords] of Object.entries(MALAYSIAN_CONFIG.MERCHANT_PATTERNS)) {
      if (keywords.some((keyword) => desc.includes(keyword))) {
        switch (pattern) {
          case 'MAMAK':
            return MalaysianCategory.FOOD_MAMAK;
          case 'GRAB':
            return desc.includes('food')
              ? MalaysianCategory.FOOD_DELIVERY
              : MalaysianCategory.TRANSPORT_GRAB;
          case 'STARBUCKS':
            return MalaysianCategory.FOOD_COFFEE;
          case 'PETROL':
            return MalaysianCategory.TRANSPORT_FUEL;
          case 'SHOPPING':
            return MalaysianCategory.SHOPPING_ONLINE;
        }
      }
    }
    ...
    if (
      amount >= MALAYSIAN_CONFIG.TYPICAL_AMOUNTS.MAMAK_MEAL.min &&
    ...
    if (
      amount >= MALAYSIAN_CONFIG.TYPICAL_AMOUNTS.GRAB_RIDE.min &&
